...
function NotificationCenter({ userId, orgId }: { userId: string; orgId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: result } = await supabase.from('notifications').select('id,title,body,read_at,created_at').eq('user_id', userId).eq('organization_id', orgId).order('created_at', { ascending: false }).limit(30)
  ...
  return <div className="notification-center"><button className="icon-btn notification" onClick={() => setOpen((value) => !value)} aria-label={`Notifications${unread ? `, ${unread} unread` : ''}`} aria-expanded={open}><Bell size={19} />{unread > 0 && <b>{unread > 9 ? '9+' : unread}</b>}</button>{open && <section className="notification-popover" role="dialog" aria-label="Notifications"><div className="notification-header"><div><strong>Notifications</strong><span>{unread ? `${unread} unread` : 'All caught up'}</span></div><button className="text-btn" onClick={() => void markAllRead()} disabled={!unread}>Mark all read</button></div>{error && <p className="notification-error">{error}</p>}{!navigator.onLine ? <p className="notification-empty">You’re offline. Reconnect to check for new notifications.</p> : !rows.length ? <p className="notification-empty">No notifications yet.</p> : <div className="notification-list">{rows.map((row) => <button className={`notification-item${row.read_at ? '' : ' unread'}`} key={row.id} onClick={() => void markRead(row.id)}><span className="notification-dot" /><span><strong>{row.title}</strong><small>{row.body}</small><time>{new Date(row.created_at).toLocaleString('en-NG')}</time></span></button>)}</div>}</section>}</div>
}
...
function Reports({ orgId }: { orgId: string }) {
  ...
  return <div className="page"><PageIntro label="Reports" title="Understand the signal." description="Server-side P&L aggregates. Historical item costs are used for COGS." /><section className="panel report-filters"><label>From<input type="date" value={dateFrom} max={dateTo} onChange={(event) => setDateFrom(event.target.value)} /></label><label>To<input type="date" value={dateTo} min={dateFrom} max={new Date().toISOString().slice(0, 10)} onChange={(event) => setDateTo(event.target.value)} /></label></section>{error && <div className="form-error" role="alert">{error}</div>}<div className="metrics"><Metric label="Revenue" value={`₦${Number(metrics?.revenue ?? 0).toLocaleString('en-NG')}`} note="Completed sales" /><Metric label="COGS" value={`₦${Number(metrics?.cogs ?? 0).toLocaleString('en-NG')}`} note="Historical sale costs" /><Metric label="Gross profit" value={`₦${Number(metrics?.gross_profit ?? 0).toLocaleString('en-NG')}`} note="Revenue less COGS" /><Metric label="Operating expenses" value={`₦${Number(metrics?.operating_expenses ?? 0).toLocaleString('en-NG')}`} note="Recorded expenses" /></div><section className="panel report-note"><BarChart3 size={22} color="#06b6d4" /><h2>Net profit</h2><strong>₦{Number(metrics?.net_profit ?? 0).toLocaleString('en-NG')}</strong><p>Revenue minus historical COGS and operating expenses.</p></section></div>
}
...
function SettingsPage({ orgId }: { orgId: string }) {
  ...
  const [announcement, setAnnouncement] = useState<{ version: string; message: string; created_at: string } | null>(null)
  ...
  useEffect(() => { supabase?.from('app_version_announcements').select('version,message,created_at').order('created_at', { ascending: false }).limit(1).maybeSingle().then(({ data }) => setAnnouncement(data)) }, [])
