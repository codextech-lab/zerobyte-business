...
function SalesRecords({ orgId, search }: { orgId: string; search: string }) {
  ...
  const load = useCallback(async () => {
    if (!supabase) return
    const { data, error: result } = await supabase.from('sales').select('id,customer_id,total,status,created_at,customer:customers(name)').eq('organization_id', orgId).order('created_at', { ascending: false }).limit(100)
