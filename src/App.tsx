...
function Sales({ orgId }: { orgId: string }) {
  const [products, setProducts] = useState<ProductRow[]>([]); const [customers, setCustomers] = useState<CustomerRow[]>([]); const [selected, setSelected] = useState(''); const [customer, setCustomer] = useState(''); const [quantity, setQuantity] = useState('1'); const [cart, setCart] = useState<{ product_id: string; quantity: number }[]>([]); const [message, setMessage] = useState('')
